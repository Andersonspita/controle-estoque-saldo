...
> **Última Atualização:** 15/08/2026 — Testes E2E legados removidos; código pronto para o repositório GitHub
...
- Dependência: `src/deps.py` (`get_current_user` / `get_current_active_user`). Token inválido ou ausente → **401**. Usuário inativo → **403**.
...
- Frontend (`frontend/src/services/api.ts`) envia `Authorization: Bearer` a partir de `localStorage.access_token`.
- Correção: `/users/me` deixou de usar uma SECRET_KEY dummy (que caía no primeiro usuário do banco).

## 7. De Onde Retomar (Próximos Passos)

1. **Perfis/autorização:** JWT autentica o usuário; ainda não há checagem fina por perfil (ADMIN vs operador) nas rotas de domínio.
2. **E2E autenticado:** o Playwright hoje só sobe o frontend; um teste de login + dashboard exigiria o backend no `webServer` (ou `docker compose`).
3. **Melhorias de NF:** OCR de PDF é mais lento que XML; quando existir o XML da NF-e, preferi-lo. Conferência visual dos vínculos NF × contrato em notas já importadas.
