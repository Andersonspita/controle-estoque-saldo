...
Ponto de restauração no Git **antes** do Relatório de Saldo de Contrato: tag `backup-pre-relatorio-saldo-20260901` (commit `9e36078`). Tags anteriores: `backup-pre-licitacao-obs-20260824`, `backup-pre-objeto-vigencia-20260824` e `backup-pre-nf-manual-20260824`. A partir de 24/08/2026, **toda alteração** cria uma tag `backup-pre-<resumo>-YYYYMMDD` no HEAD atual **antes** de editar arquivos.
